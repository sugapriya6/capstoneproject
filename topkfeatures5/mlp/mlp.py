...
# MLP uses SelectKBest (ANOVA)
# NOT RFE — MLP has no feature_importances_
# ==============================
...
for k in range(min_k, max_k + 1, step):
    ...
    y_prob = mlp.predict_proba(X_test_sel)[:, 1]
...
plt.title("MLP — Performance Metrics vs Top-K Features (ANOVA)")
...
plot_path = os.path.join(MODEL_DIR, "mlp_topk_metrics.png")
...
print(f"\nSaving final MLP model with best k = {best_k}")
...
# Train final MLP on FULL DARWIN data
final_mlp = MLPClassifier(
    hidden_layer_sizes=(100, 50),
    max_iter=500,
    random_state=42
)
final_mlp.fit(X_final, y)

joblib.dump(final_mlp,                               os.path.join(MODEL_DIR, "mlp_model.joblib"))
joblib.dump(final_selector,                          os.path.join(MODEL_DIR, "mlp_selector.joblib"))
joblib.dump(X.columns[final_selector.get_support()], os.path.join(MODEL_DIR, "mlp_features.joblib"))

print(f"mlp_model.joblib    saved")
print(f"mlp_selector.joblib saved")
print(f"mlp_features.joblib saved")
