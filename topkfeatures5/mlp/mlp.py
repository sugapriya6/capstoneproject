...
plt.legend()
plt.grid(True)
plt.savefig("C:/capstone/alzheimers/proposed/topkfeatures5/mlp/mlp_metrics.png", dpi=300, bbox_inches="tight")
...
# ==============================
# SAVE FINAL MODEL (BEST k BY MCC)
# ==============================

best_k = k_values[best_mcc_index]
print("Saving MLP model with best k =", best_k)

# Refit SelectKBest with best k
...
final_mlp = MLPClassifier(random_state=42)
final_mlp.fit(X_train_final, y_train)

# Save paths
MODEL_DIR = "C:/capstone/alzheimers/proposed/topkfeatures5/mlp/"

joblib.dump(final_mlp, MODEL_DIR + "mlp_model.joblib")
joblib.dump(final_selector, MODEL_DIR + "mlp_selector.joblib")
joblib.dump(X.columns[final_selector.get_support()], MODEL_DIR + "mlp_features.joblib")

print("✅ MLP model, selector & features saved successfully")
