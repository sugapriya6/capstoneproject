...
# Train = 80%, Test = 20%
# RFE fitted on train only
# Evaluated on unseen test
# ==============================
...
for k in range(min_k, max_k + 1, step):
    ...
    y_prob = rf.predict_proba(X_test_rfe)[:, 1]
...
plt.title("Random Forest — Performance Metrics vs Top-K Features (RFE)")
...
plot_path = os.path.join(MODEL_DIR, "rf_topk_metrics.png")
plt.savefig(plot_path, dpi=300, bbox_inches="tight")
plt.show()
print(f"\nPlot saved: {plot_path}")

# ==============================
# 7. SAVE FINAL MODEL WITH BEST K
#    Using MCC as criterion (paper method)
#    Retrain on FULL DARWIN data
# ==============================
best_k = k_values[best_mcc_idx]
print(f"\nSaving final RF model with best k = {best_k}")

final_rf  = RandomForestClassifier(n_estimators=200, random_state=42)
final_rfe = RFE(estimator=final_rf, n_features_to_select=best_k)
...
joblib.dump(final_rfe,                     os.path.join(MODEL_DIR, "rf_rfe.joblib"))
joblib.dump(X.columns[final_rfe.support_], os.path.join(MODEL_DIR, "rf_features.joblib"))

print(f"rf_model.joblib    saved")
print(f"rf_rfe.joblib      saved")
print(f"rf_features.joblib saved")
