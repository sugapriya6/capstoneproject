...
# SVM uses SelectKBest (ANOVA)
# NOT RFE — SVM has no feature_importances_
# ==============================
...
for k in range(min_k, max_k + 1, step):
    ...
    y_prob = svm.predict_proba(X_test_sel)[:, 1]
...
plt.title("SVM — Performance Metrics vs Top-K Features (ANOVA)")
...
plot_path = os.path.join(MODEL_DIR, "svm_topk_metrics.png")
...
print(f"\nSaving final SVM model with best k = {best_k}")

# Refit ANOVA on FULL DARWIN data
final_selector = SelectKBest(score_func=f_classif, k=best_k)
X_final = final_selector.fit_transform(X, y)

# Train final SVM on FULL DARWIN data
final_svm = SVC(probability=True, kernel="rbf", random_state=42)
final_svm.fit(X_final, y)

joblib.dump(final_svm,                               os.path.join(MODEL_DIR, "svm_model.joblib"))
joblib.dump(final_selector,                          os.path.join(MODEL_DIR, "svm_selector.joblib"))
joblib.dump(X.columns[final_selector.get_support()], os.path.join(MODEL_DIR, "svm_features.joblib"))

print(f"svm_model.joblib    saved")
print(f"svm_selector.joblib saved")
print(f"svm_features.joblib saved")
