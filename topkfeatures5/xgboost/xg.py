...
# XGB uses RFE (has feature_importances_)
# NOT ANOVA
# ==============================
...
for k in range(min_k, max_k + 1, step):
    ...
    rfe = RFE(estimator=xgb, n_features_to_select=k)
    ...
    y_prob = xgb.predict_proba(X_test_rfe)[:, 1]
...
plt.title("XGBoost — Performance Metrics vs Top-K Features (RFE)")
...
plot_path = os.path.join(MODEL_DIR, "xgb_topk_metrics.png")
...
print(f"\nSaving final XGB model with best k = {best_k}")
...
final_rfe = RFE(estimator=final_xgb, n_features_to_select=best_k)
...
joblib.dump(final_rfe,                     os.path.join(MODEL_DIR, "xgb_rfe.joblib"))
joblib.dump(X.columns[final_rfe.support_], os.path.join(MODEL_DIR, "xgb_features.joblib"))

print(f"xgb_model.joblib    saved")
print(f"xgb_rfe.joblib      saved")
print(f"xgb_features.joblib saved")
