...
def main():
    ...
    adaboost = AdaBoostClassifier(
        estimator=base_tree,   # sklearn >=1.2 uses "estimator"
        n_estimators=50,
        random_state=42
    )

    adaboost.fit(X_train_sel, y_train)

    # =============================
    # TESTING
    # =============================
    y_pred = adaboost.predict(X_test_sel)
    ...
    # SAVE MODEL
    # =============================
    joblib.dump(adaboost, MODEL_DIR + "adaboost_model.joblib")
    joblib.dump(rfe, MODEL_DIR + "adaboost_selector.joblib")
    joblib.dump(X.columns[rfe.support_], MODEL_DIR + "adaboost_features.joblib")
